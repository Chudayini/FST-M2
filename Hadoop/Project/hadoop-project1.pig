-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/udayini/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/udayini/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/udayini/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
STORE namesOrdered INTO 'hdfs:///user/udayini/outputs' USING PigStorage('\t');
